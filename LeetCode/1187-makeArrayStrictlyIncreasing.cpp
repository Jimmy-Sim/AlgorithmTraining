...
public:
    int makeArrayIncreasing(vector<int>& ar1, vector<int>& ar2) {
        ...
        for (int i = 1; i < n; i++) {
            for (int j = 0; j <= i + 1; j++) dp[i][j] = INT_MAX;
        }

        for (int i = 1; i < n; i++) {
            for (int j = 0; j <= i; j++) {
                if (dp[i - 1][j] < ar1[i]) dp[i][j] = min(dp[i][j], ar1[i]);
...
private:
    int _find(const vector<int>& ar, int target, int lo, int hi) {
        ...
        int mid = (lo + hi + 1) / 2;
        if (ar[mid] > target) {
            if (target >= ar[mid - 1]) return ar[mid];

            else return _find(ar, target, lo, mid - 1);
        }

        else return _find(ar, target, mid, hi);
    }
};
