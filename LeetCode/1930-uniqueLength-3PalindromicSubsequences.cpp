...
public:
    int countPalindromicSubsequence(string inputString) {
        ...
        for (int charIndex = 0; charIndex < 26; charIndex++) {
            if (minExist[charIndex] == INT_MAX || maxExist[charIndex] == INT_MIN) continue;

            unordered_set<char> uniqueCharsBetween;

            for (int j = minExist[charIndex] + 1; j < maxExist[charIndex]; j++) uniqueCharsBetween.insert(inputString[j]);
