...
public:
    int minimumTime(int n, vector<vector<int>>& relations, vector<int>& time) {
        ...
        function<int(int)> calculateTime = [&](int course) {
            ...
            for (int prereq : graph[course]) maxPrerequisiteTime = max(maxPrerequisiteTime, calculateTime(prereq));
        ...
        for (int course = 0; course < n; course++) overallMinTime = max(overallMinTime, calculateTime(course));
