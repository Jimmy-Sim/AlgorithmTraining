...
public:
    ...
    vector<double> calcEquation(vector<vector<string>>& equations, vector<double>& values, vector<vector<string>>& queries) {
        ...
        for (int i = 0; i < equations.size(); i++) {
            ...
            mp[u + "->" + v] = values[i], mp[v + "->" + u] = 1 / values[i];
        ...
        for (int i = 0; i < queries.size(); i++) {
            ...
            if (graph.find(s) == graph.end() || graph.find(e) == graph.end()) ans.push_back(-1);
            else {
                double val = 1;
                map<string, int> vis;
                bool check = false;
                ...
                if (s == e) check = true;
                else dfs(s, e, mp, graph, val, vis, check);

                if (check == true) ans.push_back(val);
