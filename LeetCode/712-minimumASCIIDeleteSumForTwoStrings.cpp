...
public:
    int get_min(string& s1, string& s2, int i, int j, vector<int>& Sufix1, vector<int>& Sufix2, vector<vector<int>>& dp) {
        ...
        int sum = 0;
        if (s1[i] == s2[j]) sum += get_min(s1, s2, i + 1, j + 1, Sufix1, Sufix2, dp);
        else sum += min(s1[i] + get_min(s1, s2, i + 1, j, Sufix1, Sufix2, dp), s2[j] + get_min(s1, s2, i, j + 1, Sufix1, Sufix2, dp));
